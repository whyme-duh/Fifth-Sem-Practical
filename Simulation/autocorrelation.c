...
int main(){
    ...
    float multi;
    for(int k = 0; k<=M; k++){
        printf("\nR %d : %f",i+k*m, numbers[i+k*m]);
        printf("\nR %d: %f",i+(k+1)*m, numbers[i+(k+1)*m]);
    ...
    if(final <= z){
        printf("\nThe given sequences are autocorrelated.(as z=1.96) as we have final answer as : %f ",final);
    }
    else{
        printf("The given sequences are not autocorrelated.");
